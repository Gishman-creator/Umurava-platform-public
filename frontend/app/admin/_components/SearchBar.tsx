...
function SearchBar() {
  ...
  const searchInstant = async (term: string) => {
    ...
    try {
      const results = await searchChallenges(term, localStorage.getItem('_id')) // Replace with actual token logic
      ...
      {isSearchOpen && searchTerm && (
        <div className="bg-gray-50 absolute top-full w-full mt-2 left-0 rounded-md overflow-hidden shadow-md">
          {isLoading ? (
            ...
            <Link
              href={`/admin/challenges/${challenge._id}/${formatTitle(challenge.title)}`}
              key={challenge._id}
